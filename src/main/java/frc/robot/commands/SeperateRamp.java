...
public class SeperateRamp extends Command
{
    private double stop;
    ...
    public SeperateRamp(double width)
    {
    ...
    {
        error = (stop - Robot.rampSeperator.getTalonRampPosition());
        if((error)*pidConst < cap)
        {
            Robot.rampSeperator.runTalonRamp((error)*pidConst);
        }
        else
        {
            Robot.rampSeperator.runTalonRamp(cap);
        }
    }
